#### ARGs-OAP Analysis for Diclofenac ####
...
list_files <- list.files(path="files", full.names=TRUE)
...
#Change column names 
dataset <- rename(dataset, "0 3" = dicloconc0rep3)
dataset <- rename(dataset, "0 2" = dicloconc0rep2)
dataset <- rename(dataset, "0 1" = dicloconc0rep1)
dataset <- rename(dataset, "6400 3" = dicloconc1rep3)
dataset <- rename(dataset, "6400 2" = dicloconc1rep2)
dataset <- rename(dataset, "6400 1" = dicloconc1rep1)
dataset <- rename(dataset, "2100 1" = dicloconc2rep1)
dataset <- rename(dataset, "2100 2" = dicloconc2rep2)
dataset <- rename(dataset, "2100 3" = dicloconc2rep3)
dataset <- rename(dataset, "710 3" = dicloconc3rep3)
dataset <- rename(dataset, "710 2" = dicloconc3rep2)
dataset <- rename(dataset, "710 1" = dicloconc3rep1)
dataset <- rename(dataset, "240 3" = dicloconc4rep3)
dataset <- rename(dataset, "240 2" = dicloconc4rep2)
dataset <- rename(dataset, "240 1" = dicloconc4rep1)
dataset <- rename(dataset, "79 3" = dicloconc5rep3)
dataset <- rename(dataset, "79 2" = dicloconc5rep2)
dataset <- rename(dataset, "79 1" = dicloconc5rep1)
dataset <- rename(dataset, "26 3" = dicloconc6rep3)
dataset <- rename(dataset, "26 2" = dicloconc6rep2)
dataset <- rename(dataset, "26 1" = dicloconc6rep1)
dataset <- rename(dataset, "8 3" = dicloconc7rep3)
dataset <- rename(dataset, "8 2" = dicloconc7rep2)
dataset <- rename(dataset, "8 1" = dicloconc7rep1)
dataset <- rename(dataset, "inoc 3" = inocR3)
dataset <- rename(dataset, "inoc 2" = inocR2)
dataset <- rename(dataset, "inoc 1" = inocR1)
...
df_melt_avg_small$Treatment <- factor(df_melt_avg_small$Treatment, levels=c("inoc", "0", "8", "26", "79", "240", "710", "2100", "6400"))
df_melt_avg$Treatment <-  factor(df_melt_avg$Treatment, levels=c("inoc", "0", "8", "26", "79", "240", "710", "2100", "6400"))
...
ggplot(df_melt_avg_noinoc, aes(x=Treatment, y=reorder(antibiotic.class, desc(antibiotic.class)), fill=Avg))+
  ...
  labs(x="Diclofenac concentration (µg/L)", y="Antibiotic Resistance Gene Class", fill = "Abundance")

ggsave("Supplementary Figure 7.tiff", height = 6, width = 8, path= "figures")
...
d_nmds <- read.csv("Transposed Dataframe_total.csv", header=TRUE)
...
ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  scale_colour_manual(values = met.brewer("VanGogh3", 9)) 

ggsave("Supplementary Figure 8.tiff", height = 8, width = 8, path = "figures")
