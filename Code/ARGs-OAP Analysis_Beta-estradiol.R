#### ARGs-OAP Analysis for 17-beta-estradiol ####
...
#Change column names 
dataset <- rename(dataset, "5400 1" = Ar1)
dataset <- rename(dataset, "5400 2" = Ar2)
dataset <- rename(dataset, "5400 3" = Ar3)
dataset <- rename(dataset, "1800 1" = Br1)
dataset <- rename(dataset, "1800 2" = Br2)
dataset <- rename(dataset, "1800 3" = Br3)
dataset <- rename(dataset, "607 1" = Cr1)
dataset <- rename(dataset, "607 2" = Cr2)
dataset <- rename(dataset, "607 3" = Cr3)
dataset <- rename(dataset, "201 1" = Dr1)
dataset <- rename(dataset, "201 2" = Dr2)
dataset <- rename(dataset, "201 3" = Dr3)
dataset <- rename(dataset, "67 1" = Er1)
dataset <- rename(dataset, "67 2" = Er2)
dataset <- rename(dataset, "67 3" = Er3)
dataset <- rename(dataset, "22 1" = Fr1)
dataset <- rename(dataset, "22 2" = Fr2)
dataset <- rename(dataset, "22 3" = Fr3)
dataset <- rename(dataset, "7 3" = Gr1)
dataset <- rename(dataset, "7 2" = Gr2)
dataset <- rename(dataset, "7 1" = Gr3)
dataset <- rename(dataset, "2 3" = Hr1)
dataset <- rename(dataset, "2 2" = Hr2)
dataset <- rename(dataset, "2 1" = Hr3)
dataset <- rename(dataset, "0 1" = Ir1)
dataset <- rename(dataset, "0 2" = Ir2)
dataset <- rename(dataset, "0 3" = Ir3)
...
df_melt_avg_small$Treatment <- factor(df_melt_avg_small$Treatment, levels=c("inoc", "0", "2", "7", "22", "67", "201", "607", "1800", "5400"))
df_melt_avg$Treatment <-  factor(df_melt_avg$Treatment, levels=c("inoc", "0", "2", "7", "22", "67", "201", "607", "1800", "5400"))
...
ggplot(df_melt_avg_noinoc, aes(x=Treatment, y=reorder(antibiotic.class, desc(antibiotic.class)), fill=Avg))+
  ...
  labs(x="17-β-estradiol concentration (µg/L)", y="Antibiotic Resistance Gene Class", fill = "Abundance")
ggsave("Supplementary Figure 9.tiff", height = 8, width = 12, path= "figures")
...
d_nmds <- read.csv("Beta-Estradiol_Transposed Dataframe_total.csv", header=TRUE)
...
data.scores$Sample <- factor(data.scores$Sample, levels=c("0", "2", "7", "22", "67", "201", "607", "1800", "5400"))
...
ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  labs(x = "NMDS1", colour = "17-β-estradiol concentration \nConcentration (µg/L)", y = "NMDS2", shape = "Type")+   
